...
import { funkcjaUpdateSchema } from "@/lib/funkcjaSchema";
...
export const PUT: APIRoute = async ({ params, request, cookies }) => {
  ...
  const parsed = funkcjaUpdateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const { error } = await supabase
    .from("funkcje")
    .update({ nazwa: parsed.data.nazwa, data_od: parsed.data.data_od, data_do: parsed.data.data_do ?? null })
    .eq("id", id);
...
export const DELETE: APIRoute = async ({ params, request, cookies }) => {
  ...
  const { error } = await supabase.from("funkcje").delete().eq("id", id);
