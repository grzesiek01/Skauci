...
import { skladkaUpdateSchema } from "@/lib/skladkaSchema";
...
export const PUT: APIRoute = async ({ params, request, cookies }) => {
  ...
  const parsed = skladkaUpdateSchema.safeParse(body);
  if (!parsed.success) {
    const message = parsed.error.issues[0]?.message ?? "Błąd walidacji";
    return new Response(JSON.stringify({ error: message }), { status: 422 });
  }

  const { rok, kwota, zaplacono, data_wplaty } = parsed.data;

  const { error } = await supabase
    .from("skladki")
    .update({ rok, kwota: kwota ?? null, zaplacono, data_wplaty: data_wplaty ?? null })
    .eq("id", id);
...
export const DELETE: APIRoute = async ({ params, request, cookies }) => {
  ...
  const { error } = await supabase.from("skladki").delete().eq("id", id);
